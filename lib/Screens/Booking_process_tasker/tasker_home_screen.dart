...
class TaskerBookingOffer {
  ...
  static TaskerBookingOffer? tryParse(dynamic payload) {
    try {
      ...
      final bookingDuration = _toDoubleOrNull(
        data?['bookingDuration'] ?? data?['BookingDuration'] ?? map['bookingDuration'],
      );
...
class TaskerSignalRMessage {
  final bool isSuccess;
  final String message;
  final dynamic result;
  final dynamic errors;

  const TaskerSignalRMessage({
  ...
  static TaskerSignalRMessage? tryParse(dynamic payload) {
    try {
      dynamic obj = payload;
      ...
      final hasBookingData = map.containsKey('bookingDetailId') ||
          map.containsKey('BookingDetailId') ||
          map.containsKey('data');

      if (hasBookingData) return null;

      final hasIsSuccess = map.containsKey('isSuccess');
      final hasMessage = map.containsKey('message');

      if (!hasIsSuccess && !hasMessage) return null;

      return TaskerSignalRMessage(
      ...
      debugPrint("❌ TaskerSignalRMessage.tryParse => $e");
      return null;
    }
  }
}
...
class TaskerDispatchHubService {
  ...
  void configure({
    ...
    if (normalizedBase.contains(':0')) {
      throw Exception("❌ Invalid baseUrl: port 0 is not allowed");
    }

    final changed =
        (_baseUrl != normalizedBase) || (_userId != normalizedUserId);
  ...
  Future<void> ensureConnected({
    ...
    if (_baseUrl == null || _userId == null || _userId!.isEmpty) {
      throw Exception("HUB: configure() first");
    }
    ...
    try {
      ...
      debugPrint("❌ HUB ensureConnected failed ($_retryCount) => $e");
  ...
  Future<void> _startInternal({required Duration timeout}) async {
    try {
      ...
      final url = _buildSafeUrl();

      debugPrint("🌐 HUB URL => $url");
      ...
      final options = HttpConnectionOptions(
        ...
        // 🔥 FORCE LONG POLLING (WORKAROUND FOR YOUR BACKEND BUG)
        transport: HttpTransportType.LongPolling,
      );

   final conn = HubConnectionBuilder()
    .withUrl(url, options: options)
    .withAutomaticReconnect() // ✅ safest
    .build();
      ...
      if (!_handlersAttached) {
        _registerHandlers(conn);
        _handlersAttached = true;
      }

      final startFuture = conn.start();
      if (startFuture == null) {
        throw Exception("HUB start() returned null");
      }

      await startFuture.timeout(timeout);

      if (conn.state != HubConnectionState.Connected) {
        throw Exception("HUB not connected: ${conn.state}");
      }
  ...
  String _buildSafeUrl() {
    ...
    // Remove invalid port 0
    if (uri.hasPort && uri.port == 0) {
      uri = uri.replace(port: null);
    }

    final cleanBase = uri.toString().replaceAll(RegExp(r'/$'), '');

    return "$cleanBase/hubs/dispatch?userId=${Uri.encodeComponent(_userId!)}";
  }

  // ================= LIFECYCLE =================
  void _wireLifecycle(HubConnection c) {
    c.onclose(({error}) {
      debugPrint("🛑 onClose => $error");
    });

    c.onreconnecting(({error}) {
      debugPrint("🔄 reconnecting => $error");
    });
...
class TaskerDispatchHubService {
  ...
  void configure({
    ...
    final changed = (_baseUrl != normalizedBase) || (_userId != normalizedUserId);
  ...
  Future<void> _startInternal({required Duration timeout}) async {
    ...
    try {
      ...
      if (conn.state != HubConnectionState.Connected) {
        throw Exception("HUB: start finished but state=${conn.state}");
      }
...
class _TaskerHomeRedesignState extends State<TaskerHomeRedesign>
  ...
  List<_Task> _mapTasks(List<TaskerDashboardTask> list) {
    return list
        .map((t) => _Task(
              title: t.title,
              date: t.date,
  ...
  Future<bool> _ensureHubConnectedNow({required String reason}) async {
    ...
    final s = _hub.state;
    if (s == HubConnectionState.Connecting || s == HubConnectionState.Reconnecting) {
      debugPrint("⏳ HUB already connecting/reconnecting reason=$reason state=$s");
      return false;
    }
    ...
    try {
      ...
      await _hub.ensureConnected(timeout: const Duration(seconds: 15));
  ...
  void _attachHubListener() {
    ...
    _hubSub = _hub.notifications.listen(
      (payload) {
        ...
        final statusMsg = TaskerSignalRMessage.tryParse(payload);
        if (statusMsg != null && statusMsg.message.trim().isNotEmpty) {
          _showStatusPopup(
            message: statusMsg.message,
            isSuccess: statusMsg.isSuccess,
          );
          return;
        }

        debugPrint("❌ unknown hub payload");
      },
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      try {
        await showDialog(
          ...
          builder: (ctx) {
            final Color accent = isSuccess ? const Color(0xFF1E8E66) : Colors.redAccent;
            final Color soft = isSuccess ? const Color(0xFFEFF8F4) : const Color(0xFFFFF1F1);
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                isSuccess ? "Booking Update" : "Booking Failed",
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            ...
                            style: ElevatedButton.styleFrom(
                              backgroundColor: isSuccess ? kPrimary : Colors.redAccent,
        ...
        debugPrint("❌ status dialog exception => $e");
      } finally {
        _statusDialogOpen = false;
        Future.delayed(const Duration(milliseconds: 500), () {
          if (mounted && _lastStatusMessage == message) {
                                  ...
                                  Expanded(
                                    child: infoTile(
                                      ...
                                      value: offer.bookingDuration == null
                                          ? "-"
                                          : "${offer.bookingDuration!.toStringAsFixed(1)} hr",
                                    ),
                                  ),
                                ],
                              ),
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<UserBookingBloc, UserBookingState>(
      listenWhen: (p, c) {
        return p.taskerDashboardStatus != c.taskerDashboardStatus ||
            p.taskerDashboardError != c.taskerDashboardError ||
            p.acceptBookingStatus != c.acceptBookingStatus ||
            p.acceptBookingMessage != c.acceptBookingMessage ||
            p.acceptBookingError != c.acceptBookingError;
      },
      listener: (context, state) {
        if (state.taskerDashboardStatus == TaskerDashboardStatus.failure &&
            (state.taskerDashboardError ?? '').trim().isNotEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(state.taskerDashboardError!)),
          );
        }

        if (state.acceptBookingStatus == AcceptBookingStatus.success &&
            (state.acceptBookingMessage ?? '').trim().isNotEmpty) {
          _showStatusPopup(
            message: state.acceptBookingMessage!,
            isSuccess: true,
          );
        }

        if (state.acceptBookingStatus == AcceptBookingStatus.failure &&
            (state.acceptBookingError ?? '').trim().isNotEmpty) {
          _showStatusPopup(
            message: state.acceptBookingError!,
            isSuccess: false,
          );
        }
      },
      builder: (context, state) {
        final dash = state.taskerDashboardResponse?.result;

        final double rating = (dash?.rating ?? 0).toDouble();
        final int reviews = dash?.reviews ?? 0;
        final int acceptanceRate = dash?.acceptanceRate ?? 0;
        final int completionRate = dash?.completionRate ?? 0;

        final int weeklyEarning = dash?.weeklyEarning ?? 0;
        final int monthlyEarning = dash?.monthlyEarning ?? 0;

        final upcoming = _mapTasks(dash?.upcoming ?? const []);
        final current = _mapTasks(dash?.current ?? const []);
        ...
        return Stack(
          children: [
            ...
            if (state.taskerDashboardStatus == TaskerDashboardStatus.loading)
              Positioned.fill(
                child: IgnorePointer(
                  ignoring: false,
                  child: Container(
          ...
          children: [
            SizedBox(
              ...
              child: _KpiTile(
                ...
                title: rating == 0 ? "-" : rating.toStringAsFixed(1),
                sub: "$reviews reviews",
              ),
            ),
            SizedBox(
      ...
      children: [
        InkWell(
          ...
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(
              color: period == 'Week' ? kPrimary : kPrimary.withOpacity(0.06),
              borderRadius: BorderRadius.circular(999),
            ...
            child: Text(
              ...
              style: TextStyle(
                fontFamily: 'Poppins',
                fontWeight: FontWeight.w800,
                color: period == 'Week' ? Colors.white : kPrimary,
                fontSize: 12,
              ),
            ),
          ),
        ...
        InkWell(
          ...
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(
              color: period == 'Month' ? kPrimary : kPrimary.withOpacity(0.06),
              borderRadius: BorderRadius.circular(999),
              border: Border.all(color: kPrimary.withOpacity(0.2)),
            ),
            child: Text(
              ...
              style: TextStyle(
                fontFamily: 'Poppins',
                fontWeight: FontWeight.w800,
                color: period == 'Month' ? Colors.white : kPrimary,
                fontSize: 12,
              ),
            ),
          ),
...
//       final hasBookingData = map.containsKey('bookingDetailId') || map.containsKey('BookingDetailId') || map.containsKey('data');
//       if (hasBookingData) return null;
...
//     try {
//       _disposeConnectionOnly();
//       _conn = HubConnectionBuilder().withUrl(url, options: options).withAutomaticReconnect().build();
...
//   List<_Task> _mapTasks(List<TaskerDashboardTask> list) {
//     return list
//         .map((t) => _Task(title: t.title, date: t.date, time: t.time, location: t.location))
//         .toList(growable: false);
//   }
