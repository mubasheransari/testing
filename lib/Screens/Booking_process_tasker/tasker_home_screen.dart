...
class _TaskerHomeRedesignState extends State<TaskerHomeRedesign> with WidgetsBindingObserver {
  ...
  void _startConnectivityWatcher() {
    ...
    _netSub = Connectivity().onConnectivityChanged.listen((results) async {
      ...
      // ✅ Back online -> refresh dashboard + connect hub
      _fetchDashboard();
      await _ensureHubConnectedNow(reason: "net-back-online");
  ...
  Future<void> _dispatchLocationUpdateToApi() async {
    ...
    // TODO: replace with your real GPS values if already implemented
    const double lat = 67.00;
    const double lng = 70.00;
  ...
  /// ✅ NOW populated from API state
  /// ===============================================================
  @override
  Widget build(BuildContext context) {
    return BlocConsumer<UserBookingBloc, UserBookingState>(
      listenWhen: (p, c) =>
          p.taskerDashboardStatus != c.taskerDashboardStatus || p.taskerDashboardError != c.taskerDashboardError,
      listener: (context, state) {
        if (state.taskerDashboardStatus == TaskerDashboardStatus.failure &&
            (state.taskerDashboardError ?? '').trim().isNotEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
      ...
      builder: (context, state) {
        ...
        return Stack(
          children: [
            ...
            // ✅ loader overlay for dashboard fetch
            if (state.taskerDashboardStatus == TaskerDashboardStatus.loading)
              Positioned.fill(
                child: IgnorePointer(
                  ignoring: false,
...
//  (I kept your same widgets below — only KPI uses dynamic values)
/// ===============================================================
class _StatusPill extends StatelessWidget {
  const _StatusPill({required this.available});
  final bool available;
...
class _EarningsCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // ✅ If you already have your earnings UI, paste it here.
    // ✅ This keeps file compiling and still shows dynamic amount.
    return Row(
      children: [
        InkWell(
          onTap: () => onChange('Week'),
