...
class _TaskerHomeRedesignState extends State<TaskerHomeRedesign> {
  ...
  void initState() {
    ...
    _hubService = LocationHubService(
      hubUrl:
          'http://192.3.3.187:85/hubs/dispatch?userId=46c79690-6b69-459e-8d01-06043d4190ac',
    );
  }
  ...
  Future<void> _onAvailabilityToggle(bool value) async {
    ...
    // turning ON → mark as available and start continuous SignalR updates
    print('✅ Turning ON — starting location updates via SignalR');
  ...
  void _dispatchLocationUpdateToApi() {
    ...
    const userId = '46c79690-6b69-459e-8d01-06043d4190ac';
  ...
  /// start periodic SignalR calls
  Future<void> _startLocationUpdates() async {
    print('▶️ _startLocationUpdates called');

    // 1) Connect hub first
    try {
      await _hubService.start();
    } catch (e) {
      print('🔥 Error connecting hub: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to connect SignalR hub: $e')),
        );
      }
      // rollback toggle if hub fails
      setState(() => available = false);
      return;
    }

    // ✅ HUB CONNECTED → immediately call REST API via Bloc
    _dispatchLocationUpdateToApi();

    // 3) cancel any previous timer
    if (_locationTimer != null) {
      print('⏹ Existing timer found — cancelling');
  ...
  /// stop periodic SignalR calls
  void _stopLocationUpdates() {
    print('🛑 _stopLocationUpdates called');
    if (_locationTimer != null) {
      print('⏹ Cancelling timer');
      _locationTimer?.cancel();
      _locationTimer = null;
    } else {
      print('ℹ️ No timer to cancel');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, c) {
        ...
        final radius =
            compact ? 16.0 : (wide ? 24.0 : _TaskerHomeRedesignState.kRadius);
...
//     // NOTE: "update/location" must match the method name in your C# hub
//     await _connection.invoke(
//       'update/location',
//       args: [payload],
//     );
...
//     // send once immediately (hub is connected here)
//     print('📡 Sending immediate location update via SignalR');
//     await _sendLocationUpdateViaHub();

//     // then repeat every _locationInterval
//     _locationTimer = Timer.periodic(_locationInterval, (timer) {
//       print(
//           '⏰ Timer tick #${timer.tick} at ${DateTime.now()} — sending location via hub');
//       _sendLocationUpdateViaHub();
...
//   /// actual SignalR invocation + REST API via Bloc
//   Future<void> _sendLocationUpdateViaHub() async {
//     // your hardcoded values (replace with GPS later)
//     const userId = '46c79690-6b69-459e-8d01-06043d4190ac';
//     const double lat = 24.435;
//     const double lng = 67.435;

//     if (!_hubService.isConnected) {
//       print('⚠️ Hub not connected, skipping location send');
//       return;
//     }

//     try {
//       // 1️⃣ Send to SignalR hub
//       await _hubService.sendLocation(
//         userId: userId,
//         latitude: lat,
//         longitude: lng,
...
//       print('✅ Location sent via hub, now updating REST API via Bloc');

//       // 2️⃣ Also hit REST API: /api/Address/update/location
//       if (!mounted) return;
...
//           );
//     } catch (e) {
//       print('🔥 Error sending location via hub/API: $e');
//       if (mounted) {
//         ScaffoldMessenger.of(context).showSnackBar(
...
//                             child: _EarningCard(
//                               amount:
//                                   period == 'Week' ? weeklyEarning : monthlyEarning,
//                               sub:
//                                   'Earnings per ${period.toLowerCase()}',
//                               period: period,
//                               onChangePeriod: (p) =>
//                                   setState(() => period = p),
//                             ),
...
//                     child: ConstrainedBox(
//                       constraints:
//                           const BoxConstraints.tightFor(width: 160, height: 44),
//                       child: trailing!,
//                     ),
//                   ),
//                 ),
...
/*
/// Simple SignalR service to send location
class LocationHubService {
  final String hubUrl;
  late final HubConnection _connection;
  ...
  Future<void> sendLocation({
    ...
    // "UpdateLocation" must match the method name in your C# hub
    await _connection.invoke(
      'update/location',
      args: [payload],
    );
...
class _TaskerHomeRedesignState extends State<TaskerHomeRedesign> {
  ...
  void initState() {
    super.initState();
    _hubService = LocationHubService(
      hubUrl:'http://192.3.3.187:85/hubs/dispatch?userId=46c79690-6b69-459e-8d01-06043d4190ac' //'http://192.3.3.187:85/api/Address/update/location',
    );
  }
  ...
  Future<void> _onAvailabilityToggle(bool value) async {
    ...
    if (!value) {
      ...
      setState(() => available = false);
      _stopLocationUpdates();
      // optional: keep connection alive if you want
      await _hubService.stop();
      return;
    }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('You are now Available. Sending location via SignalR...')),
      );
    }
  }
  ...
  Future<void> _startLocationUpdates() async {
    ...
    // then repeat every _locationInterval
    _locationTimer =
        Timer.periodic(_locationInterval, (timer) {
      print('⏰ Timer tick #${timer.tick} at ${DateTime.now()} — sending location via hub');
      _sendLocationUpdateViaHub();
    });
  ...
  void _onDirectionTap() {
    // TODO: update import to your real TaskAlertGlassScreen
    // Navigator.push(
    //   context,
    //   MaterialPageRoute(
    //     builder: (context) => const TaskAlertGlassScreen(),
    //   ),
    // );
  }
}
    ...
    return LayoutBuilder(
      builder: (context, c) {
        ...
        final radius = compact
            ...
            : (wide
                ? 24.0
                : _TaskerHomeRedesignState.kRadius);
        final padAll = compact ? 12.0 : (wide ? 18.0 : 14.0);
        final marginV = compact ? 4.0 : 6.0;
        final blur = wide ? 22.0 : 18.0;
        final offsetY = wide ? 10.0 : 8.0;
...
class _PrimaryButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // FIX: give the button a finite width inside Row
    return ConstrainedBox(
      constraints: const BoxConstraints.tightFor(
        width: 160,
        height: 44,
...
//     if (!value) {
//       // turning OFF → stop location updates
//       print('⛔ Turning OFF — stopping location timer');
//       setState(() => available = false);
//       _stopLocationUpdates();
//       return;
//     }

//     // turning ON → mark as available and start continuous updates
//     print('✅ Turning ON — starting location updates');
//     setState(() => available = true);
//     _startLocationUpdates();

//     if (mounted) {
//       ScaffoldMessenger.of(context).showSnackBar(
//         const SnackBar(content: Text('You are now Available. Sending location...')),
//       );
//     }
//   }
...
//     // send once immediately
//     print('📡 Sending immediate location update');
//     _sendLocationUpdate();

//     // then repeat every _locationInterval
//     _locationTimer =
//         Timer.periodic(_locationInterval, (timer) {
//       print('⏰ Timer tick #${timer.tick} at ${DateTime.now()} — sending location');
//       _sendLocationUpdate();
//     });
...
//   /// actual API call to update location
//   Future<void> _sendLocationUpdate() async {
//     const String endpoint =
//         'http://192.3.3.187:85/api/Address/update/location';

//     print('🌍 _sendLocationUpdate → calling $endpoint');

//     try {
//       final uri = Uri.parse(endpoint);

//       final Map<String, dynamic> payload = {
//         "userId": "46c79690-6b69-459e-8d01-06043d4190ac",
//         "latitude": 24.435,
//         "longitude": 67.435,
//       };
...
//         print('❌ Location update failed: ${res.statusCode}');
//         if (mounted) {
//           ScaffoldMessenger.of(context).showSnackBar(
//             SnackBar(
//               content: Text(
//                 'Failed to send location (${res.statusCode})',
//               ),
//             ),
//           );
//         }
...
//       }
//     } catch (e) {
//       print('🔥 Exception in _sendLocationUpdate: $e');
//       if (mounted) {
//         ScaffoldMessenger.of(context).showSnackBar(
//           SnackBar(
//             content: Text('Error while sending location: $e'),
//           ),
//         );
//       }
//     }
...
//   @override
//   void dispose() {
//     print('🧹 dispose() called — cancelling timer');
//     _locationTimer?.cancel();
//     super.dispose();
//   }
